...
var formBuilderpage = {
		...
		if (type == 0){
			...
			if (type == 'checkboxes' || type == 'multiple_choices'){
				...
				if (typeof opt.display != 'undefined'){
					...
					if(typeof opt.choicesBorderRadius != 'undefined' && opt.choicesBorderRadius){
						$slide.find('input[name="choice_survey_border_radius"]').val(opt.choicesBorderRadius);
					}
				}
	...
	actionStyleField: function(){
		...
		$('input[name="choice_survey_border_radius"]').keyup(function(){
			var $tpl = $('.tpl-block.selected');
			var opt = $tpl.data('json');
			
			opt.choicesBorderRadius = $(this).val();
			formBuilderpage.set_elemenet_data($tpl, opt)
	...
	updateChoicesStyleInline: function($tpl, opt){
		...
		var cssBorderRadius = '#'+id+' .tpl-choices .t-field:nth-child(3) label{border-radius: '+opt.choicesBorderRadius+'px 0 0 '+opt.choicesBorderRadius+'px;} #'+id+' .tpl-choices .t-field:last-child label{border-radius: 0 '+opt.choicesBorderRadius+'px '+opt.choicesBorderRadius+'px 0;} @media screen and (max-width: 639px){#'+id+' .tpl-choices .t-field:nth-child(3) label{border-radius: '+opt.choicesBorderRadius+'px '+opt.choicesBorderRadius+'px 0 0;} #'+id+' .tpl-choices .t-field:last-child label{border-radius: 0 0 '+opt.choicesBorderRadius+'px '+opt.choicesBorderRadius+'px;}}';
		var css = '#'+id+' .t-field label:hover, #'+id+' .t-field .checked label {background-color: '+opt.choicesSelectedBgColor+'!important; border-color: '+opt.choicesSelectedBorderColor+'!important;} #'+id+' .t-field label:hover .fb-choice, #'+id+' .t-field .checked label .fb-choice {color: '+opt.choicesSelectedFontColor+'!important;}' + cssBorderRadius;
