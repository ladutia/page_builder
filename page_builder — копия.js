var pageBuilder = {
    ...
    init: function () {
        ...
        $('.pb-list-image').on('click', '.pb-image__link-browse', function () {
            ...
            pageBuilder.openUploader(function(url){
                pageBuilder.insertImage(that.parents('.pb-image__item'), true, url);
            });
        });
        ...
        $('.upload_page_favicon').click(function () {
            ...
            pageBuilder.openUploader(function (url) {
                ...
                opt.favicon = url;
                $tpl.attr('data-json', JSON.stringify(opt));
                var fileName = pageBuilder.getBgImageName(url);
    ...
    dropFreeImages: function ($el) {
        var $el = $el ? $el : $('.pb-widget:not(.pb-widget--svg):not(.pb-widget--icon):not(.pb-widget--field):not(.pb-widget--video):not(.pb-widget--button):not(.pb-widget--code)');
    ...
    addItemTree: function (type, idx, $parent) {
        ...
        if (type === 'nav-items' || type === 'container' || type === "two-column-grid" || type === "three-column-grid" || type === "uneven-grid" || type === "vertical-form" || type === "horizontal-form" || type === "custom-form")
            isNestedList = '';
    ...
    getHTMLElement: function (type, $el) {
        ...
        if (type === 'container' && !type2) {
            ...
            dataJson = '{"containerBackground":"#ffffff", "backgroundImageUrl":"", "containerBorderType":"None", "containerBorderWidth":"0", "containerBorderColor":"#ffffff", "btnBackground":"#fafafa", "btnBorderType":"Solid", "btnBorderWidth":"1", "btnBorderColor":"#cccccc", "btnBorderRadius":"5", "fontTypeFace": "Arial", "fontWeight":"Normal", "fontSize":"12", "color":"#505050", "lineHeight":"None","align": "0", "width":"1","styleIcon":"0", "layout": "1", "contentToShare":"1", "shareCustomUrl":"0", "shareLink":"%%webversion_url_encoded%%", "shareDesc":""}';
            ...
            html = "<div class='pb-widget pb-widget--init pb-widget--container pb-widget--header-mode pb-container-contains-widget' data-type2='header-1' data-type='" + type + "' data-json='" + dataJson + "' style='padding: 10px 15px;'>" +
                ...
                '<div class="pb-widget__content pb-svg-box"><div class="pb-load-svg"><svg fill="#fb8f04" height="512px" id="Layer_1" style="enable-background:new 0 0 512 512;" version="1.1" viewBox="0 0 512 512" width="512px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g><path d="M417.4,224H94.6C77.7,224,64,238.3,64,256c0,17.7,13.7,32,30.6,32h322.8c16.9,0,30.6-14.3,30.6-32   C448,238.3,434.3,224,417.4,224z"/><path d="M417.4,96H94.6C77.7,96,64,110.3,64,128c0,17.7,13.7,32,30.6,32h322.8c16.9,0,30.6-14.3,30.6-32   C448,110.3,434.3,96,417.4,96z"/><path d="M417.4,352H94.6C77.7,352,64,366.3,64,384c0,17.7,13.7,32,30.6,32h322.8c16.9,0,30.6-14.3,30.6-32   C448,366.3,434.3,352,417.4,352z"/></g></svg></div></div>' +
                pageBuilder.getWidgetHTMLPlaceholder() +
                pageBuilder.getWidgetHTMLLabel('svg') +
                '</div>' +
                    //END SVG Toggle
        ...
        if (!$el.hasClass('pb-widget')) {
            switch(type){
                case 'custom-form':
                    ...
                    if(! LandingPageBuilder.form_id){
                        $el.hide();
                        LandingPageBuilder.open_custom_forms_popup();
                    } else {
                        show_error_message("Only one webform could be added into a landing page");
    ...
    updateIndividualOptions: function () {
        ...
        $('#containerBorderRadius').change(function () {
            getTplOpt();
            opt.borderRadius = $(this).val();
            $tpl.children('.pb-container-grid').css('border-radius', opt.borderRadius + 'px');
            $tpl.attr('data-json', JSON.stringify(opt));
        });
    ...
    loadImage: function () {
        $('.pb-blocks').off('dblclick.loadImage').on('dblclick.loadImage', '.pb-load-image', function (e) {
            ...
            var that = $(this);
            pageBuilder.openUploader(function (url) {
                pageBuilder.insertImage(that, false, url);
            });
        });
    },
    loadSvg: function () {
    ...
    insertImage: function ($box, isBrowse, urlImage) {
        var src = urlImage ? urlImage : 'https://s3.amazonaws.com/ll-demo-001/media-manager-file-uploads/customers/11295/root/professionalservices.jpg';
        var $tpl = $('.pb-widget--selected');
    ...
    socialIconAction: function () {
        ...
        $('.pb-list-group-social').on('keyup', '.pb-field-social-text', function () {
            var $tpl = $('.pb-widget--selected');
            var $li = $(this).parents('li');
            var typeIcon = $li.attr('idx');
            var $btn = $tpl.find('.pb-social-btn').eq($li.attr('datasortid'));
    ...
    addBgImage: function ($box, isGlobal, url, $tpl) {
        ...
        var opt = null;
        var url = url ? url : "https://images.unsplash.com/photo-1478760329108-5c3ed9d495a0";
    ...
    openUploader: function(onInsertCallback){
        if(_moxiemanager_plugin){
            moxman.browse({
                view: 'thumbs',
                title: 'Media Manager',
                ...
                oninsert: function(args) {
                    if(args.files[0].url){
                        onInsertCallback(args.files[0].url);
                    }
                }
            });
        } else {
                ...
                onsave: function(args) {
                    if(args.file.url){
                        ...
                        if(img.canPreview){
                            ...
                            pageBuilder.insertImage($el, true, urlImage);
                        }
                    }
                }
            });
